...
SystemExec PoseCommitSystem::update(GameContext* context) {
    ...
    for (auto [e, cPose] : context->registry.getEntitiesWithComponents<CPose>())
    {
        ...
        auto [cRacketTransform3D, cRacketPhysical, cRacketVelocity3D] = context->registry.getComponents <CTransform3D, CRacketPhysical, CVelocity3D > (eRacket);
        ...
        // velocity must match transform delta
