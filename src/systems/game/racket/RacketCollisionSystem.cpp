...
SystemExec RacketCollisionSystem::update(GameContext* context)
{
    ...
    for (auto eBall : ballEntities)
    {
        ...
        for (auto eRacket : racketEntities)
        {
            ...
            Vec3 vTanRel = vTanRacket - vTanBall;
            ...
            if (tanSpeed > 0.0001f)
            {
                ...
                Vec3 racketRight = n.cross(racketUp);
                ...
                float jtUp = std::clamp(std::abs(slipUp) * 0.15f, 0.0f, jtMax);
                float jtSide = std::clamp(std::abs(slipSide) * 0.10f, 0.0f, jtMax);
            ...
            // 8. Final velocity
            cBallVelocity3D->vel_mps = vNormalOut + vTanBall + push + vTanRel * 0.25f;
            cBall->spin += spinRev * powerMult;
